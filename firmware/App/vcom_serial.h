...
typedef struct {
	uint8_t  rx_buff[RX_BUFF_SIZE] __ALIGNED(4);
	uint16_t rx_bytes;
	uint16_t rx_wrptr;
	uint16_t rx_rdptr;

	uint8_t  tx_buff[TX_BUFF_SIZE] __ALIGNED(4);
	uint16_t tx_bytes;
	uint16_t tx_wrptr;
	uint16_t tx_rdptr;
	...
	uint8_t  in_buff[USB_BUFF_SIZE] __ALIGNED(4);
	uint16_t in_bytes;
	uint16_t in_ready;

	uint8_t  out_buff[USB_BUFF_SIZE] __ALIGNED(4);
	uint16_t out_bytes;
	uint16_t out_ready;
} VCOM;
